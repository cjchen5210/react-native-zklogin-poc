...
export async function getSaltFromEnoki(jwtEncoded: string, enokiToken: string) {
    const url = "https://api.enoki.mystenlabs.com/v1/zklogin";
    // const decodedJwt: LoginResponse = jwt_decode(jwtEncoded) as LoginResponse;
    // console.log("decodedJwt:", decodedJwt);

    const config = {
        headers: { Authorization: `Bearer ${enokiToken}`, "zklogin-jwt": jwtEncoded }
    };

    // console.log("Getting salt:", url, config);
    const res = await axios.get(url, config);

    // console.log("Salt Response:", res.data);
...
export async function getZNPFromEnoki(jwtToken: string, userKeyData: UserKeyData, enokiToken: string, forceUpdate = true): Promise<ZkLoginSignatureInputs> {

    const url = "https://api.enoki.mystenlabs.com/v1/zklogin/zkp";
    const decodedJwt: LoginResponse = jwt_decode(jwtToken) as LoginResponse;
    // const ephemeralPrivateKeyArray = Uint8Array.from(Array.from(fromB64(userKeyData.ephemeralPrivateKey!)));
    // const ephemeralKeyPair = Ed25519Keypair.fromSecretKey(ephemeralPrivateKeyArray);
    const config = {
        headers: { Authorization: `Bearer ${enokiToken}`, "zklogin-jwt": jwtToken }
    };
    ...
    const zkpPayload =
    {
        ephemeralPublicKey: userKeyData.ephemeralPublicKeyRaw,
        randomness: userKeyData.randomness,
        maxEpoch: userKeyData.maxEpoch,
    };
    ...
    const proofResponse = await axios.post(url, zkpPayload, config)
    ...
    return proofResponse?.data.data;
}
